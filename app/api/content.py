...
from app.utils.chunker import CHUNK_SIZE, chunk_text
...
def list_contents():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT content_id, filename, LENGTH(text)/? AS chunks, created_at FROM content",
        (CHUNK_SIZE,),
    )
    rows = c.fetchall()
    conn.close()
...
def view_content(content_id: str):
    ...
    text = row[0]
    c.execute(
        "SELECT chunk_id, text_chunk FROM chunks WHERE content_id=? ORDER BY chunk_id",
        (content_id,),
    )
